...
const AppInner: React.FC = () => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "notifications", "latest"), (snap) => {
      if (!snap.exists()) return;
      const data = snap.data();

      const lastSeen = localStorage.getItem('lastSeenNotifVersion');
      if (data.version > Number(lastSeen ?? 0)) {
        setNotif({ title: data.title, message: data.message });
        localStorage.setItem('lastSeenNotifVersion', String(data.version));
      }
    });
    return () => unsub();
  }, []);
    ...
    {notif && (
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '12px', opacity: 0.75, fontWeight: 600 }}>
            {notif.message}
          </div>
        </div>
        <button
          ...
          style={{ background: 'none', border: 'none', color: '#fff',
            fontSize: '18px', cursor: 'pointer', opacity: 0.6, padding: '0 4px' }}
        >
          ✕
        </button>
      </div>
